...
var m_data_get = (function () {
 ...
 m_data_get.getEvents = function(do_next_func) {                                                                                                
   m_data_get.ensureState(); // ADD: missing call                                                                                              
  ...
  // [PROPOSED UPDATE]: Fetch details of a single event by its ID from API */
  ...
  m_data_get.getSignedInFromUser = function(UID, do_next_func) {
    $.ajax({
      method: "GET",
      url: fullPathSignIn +"/user/" + UID,
      headers: _authHeaders(),
      contentType: "application/json",
      success: function (result) {
          var signIns = result !== null ? result : [];
          eventsData.finishedWithError = false;

        if (signIns.length === 0) {
          eventsData.data = signIns;
          if (typeof do_next_func === "function") {
            do_next_func();
          }
          return;
        ...
        var remaining = signIns.length;
        for (var i = 0; i < signIns.length; i++) {
          // Fügt die Eventdetails zu jedem Eintrag hinzu, damit sie in der UI angezeigt werden können
          (function(idx) {
            var entry = signIns[idx];
            m_data_get.getEventData(entry.EID, function() {
              entry.name = eventsData.data.name;
              entry.start = eventsData.data.start;
              entry.end = eventsData.data.end;
              remaining--;
              if (remaining === 0) {
                eventsData.data = signIns;
                if (typeof do_next_func === "function") {
  ...
  /*Register the current user for a specific event by its ID (SignInController) || Fertig getestet*/
  m_data_get.joinEvent = function(EID, do_next_func) {
    ...
    $.ajax({
      ...
      success: function (data) {
        if (typeof do_next_func === "function") {
          console.log(EID + " beigetreten");
          do_next_func(null, data);
        }
      },
      error: function (xhr, status, error) {
        if (typeof do_next_func === "function"){
          console.log(error.message);
           do_next_func({xhr: xhr, status: status, error: error});
        } 
      }
    });
  ...
  /* [PROPOSED UPDATE]: Create a new event record (EventController) || Fertig getestet*/
  ...
  /* [PROPOSED UPDATE]: Permanently remove an event record (EventController) || Fertig getestet */
  ...
  /* [PROPOSED UPDATE]: Update the details of an existing event record (EventController) || Fertig getestet */
